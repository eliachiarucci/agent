...
- **Deployment / open source install:** end users install via the `agent` CLI (separate repo, `../agent-cli`) — it renders a compose stack (this image + `../agent-ui`'s nginx image + pgvector + SearXNG) from `~/.agent/config.json`; see [docs/install.md](docs/install.md). `GET /agent/health` (`api/agent/health.ts`, unauthenticated DB ping) serves the container healthcheck and the CLI's readiness wait. `TRANSFORMERS_CACHE` redirects the embedding-model cache to a volume in containers.
