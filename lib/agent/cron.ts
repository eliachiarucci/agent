...
import { resolveDefaultModelTarget } from "./default-model";
...
async function resolveJobModel(job: CronJob) {
  if (job.provider) {
    ...
    if (setting && modelId) {
      return {
        model: chatModelFromSettings(job.provider, setting.settings, modelId),
        provider: job.provider,
        modelId,
      };
    }
  }
  const target = await resolveDefaultModelTarget(job.userId);
  if (!target) return null;
  return {
    model: chatModelFromSettings(target.provider, target.settings, target.model),
...
async function executeCronJob(
  ...
  const agentPrompt = agent.systemPrompt?.trim()
    ...
    : "";
  const resolved = await resolveJobModel(job);
  if (!resolved) {
    throw new Error(
  ...
  // connected connectors only, filtered by their per-model toggles.
