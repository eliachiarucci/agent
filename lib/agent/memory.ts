...
export async function buildMemorySystemPrompt(queryText: string): Promise<string> {
  const [pinned, retrieved] = await Promise.all([
    getPinnedMemories(),
    searchMemories(queryText, { limit: 8 }),
  ]);
  ...
  if (retrieved.length > 0) {
    sections.push(
      `## Memories possibly relevant to this conversation\n${retrieved.map(formatMemory).join("\n")}`
    );
  }
  ...
  sections.push(
    [
      ...
      "- If something contradicts an existing memory, use updateMemory or forget with the id shown above instead of storing a duplicate.",
